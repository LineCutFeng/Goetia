...
import com.lcf.goetia.choreographerhook.common.Constants;
import com.lcf.goetia.demo.R;

import java.util.LinkedList;

public class FrameBarChartView extends View {
    ...
    public FrameBarChartView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public FrameBarChartView(Context context, AttributeSet attrs) {
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (changed) {
            ...
            //分割成maxFps段
            float rate = lineContentHeight / maxFps;
    ...
    class LineInfo {
        ...
        LineInfo(int fps, int color) {
            ...
            linePoint[0] = height;
            // endY
            linePoint[2] = (maxFps - fps) * (lineContentHeight) / maxFps + (getHeight() - lineContentHeight);
    ...
    private int getColor(int fps) {
        int color;
        float lostRatio = 1 - ((float)fps) / maxFps;
        if (lostRatio < Constants.DEFAULT_DROPPED_NORMAL) {
            color = bestColor;
        } else if (lostRatio < Constants.DEFAULT_DROPPED_MIDDLE) {
            color = normalColor;
        } else if (lostRatio < Constants.DEFAULT_DROPPED_HIGH) {
            color = middleColor;
        } else if (lostRatio < Constants.DEFAULT_DROPPED_FROZEN) {
            color = highColor;
        } else {
            color = frozenColor;
        }
